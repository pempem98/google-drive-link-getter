...
const HistoryTab: React.FC<HistoryTabProps> = ({
  ...
  const handleHistoryExportClick = (entry: HistoryEntry, index: string) => {
    ...
    performExport(linksToExport, entry.separator, `history-${index}`);
  };


  return (
    <div className="history-container"> {/* ... input tìm kiếm và sort controls ... */}
      ...
      {sortedHistory.length === 0 ? ( <p>{getMessage(messages, 'noHistory')}</p> ) : (
        sortedHistory.map((entry, index) => (
          <div key={index} className="history-entry">
            ...
            <pre>{getDisplayTextForHistoryEntry(entry)}</pre> {/* Hiển thị nội dung đã xử lý */}
          </div>
        ))
      )}
    </div>
